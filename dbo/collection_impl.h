 ...
 */
#ifndef DBO_COLLECTION_IMPL_H_
#define DBO_COLLECTION_IMPL_H_
...
struct Helper
{
	static void skipIfRemoved(typename collection<C>::iterator::shared_impl& it)
	{
	}
};
...
struct Helper<ptr<T> >
{
	static void skipIfRemoved(typename collection<ptr<T> >::iterator::shared_impl& it)
	{
		if(std::find(it.collection_.manualModeRemovals().begin(), it.collection_.manualModeRemovals().end(), it.current_)!=it.collection_.manualModeRemovals().end())
			it.fetchNextRow();
	}
};
}
...
{
	return impl_==other.impl_||(!impl_&&other.impl_->ended_)||(impl_->ended_&&!other.impl_);
}
...
template<class C>
collection<C>::iterator::shared_impl::shared_impl(const collection<C>& collection, SqlStatement *statement)
...
void collection<C>::iterator::shared_impl::fetchNextRow()
{
	...
	{
		int column = 0;
		current_ = query_result_traits<C>::load(*collection_.session(), *statement_, column);
...
template<class C>
collection<C>::iterator::iterator(const collection<C>& collection, SqlStatement *statement)
{
...
template<class C>
C collection<C>::const_iterator::operator*()
{
...
template<class C>
bool collection<C>::const_iterator::operator==(const const_iterator& other) const
{
...
template<class C>
typename collection<C>::const_iterator collection<C>::const_iterator::operator++(int)
{
	impl_++;
...
template<class C>
collection<C>::const_iterator::const_iterator(const collection<C>& collection, SqlStatement *statement)
...
template<class C>
collection<C>::collection(Session *session, SqlStatement *statement, SqlStatement *countStatement)
		: session_(session),
...
Query<C, DynamicBinding> collection<C>::find() const
{
	...
	if(session_&&data_.relation.sql)
	{
		...
		Query<C, DynamicBinding> result = Query<C, DynamicBinding>(*session_, tableName, "").where(sql->substr(w+7));
...
void collection<C>::insert(C c)
{
	...
	{
		SetReciproceAction setPtr(session_, relation.setInfo->joinName, relation.dbo);
...
void collection<C>::erase(C c)
{
	...
	typename std::vector<C>::iterator pos = std::find(manualModeInsertions_.begin(), manualModeInsertions_.end(), c);
...
void collection<C>::clear()
{
	...
	if(relation.dbo)
	{
		...
		if(relation.setInfo->type==ManyToMany)
		{
			...
			std::size_t w = Impl::ifind(*sql, " where ");
			deleteSql = "delete from "+sql->substr(j+5, o-j-5)+sql->substr(w);
		}
		else
		{
			std::size_t f = Impl::ifind(*sql, " from ");
...
int collection<C>::count(C c) const
{
	...
	Impl::MappingInfo *mapping = session_->getMapping(relation.setInfo->tableName);

	Query<C, DynamicBinding> q = find().where(mapping->idCondition);
	c.obj()->bindId(q.parameters_);
	int result = q.resultList().size();

	result += std::count(manualModeInsertions_.begin(), manualModeInsertions_.end(), c);
	result -= std::count(manualModeRemovals_.begin(), manualModeRemovals_.end(), c);
	return result;
}
