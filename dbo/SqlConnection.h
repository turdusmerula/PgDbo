 ...
 */
#ifndef DBO_SQL_CONNECTION_H_
#define DBO_SQL_CONNECTION_H_
 ...
 *
 * Oracle is using Rownum and Firebeard is using RowsFromTo instead of limit and
 * offset in sql
 */
enum LimitQuery
{
...
/*! \class SqlConnection Dbo/SqlConnection Dbo/SqlConnection
 ...
 * id's.
 *
 * This class is part of Dbo's backend API, and should not be used
 * directly.
 *
...
public:
	...
	autoincrementCreateSequenceSql(const std::string &table, const std::string &id) const = 0;
	 ...
	 */
	virtual std::vector<std::string>
	autoincrementDropSequenceSql(const std::string &table, const std::string &id) const = 0;
	...
	/*! \brief Returns the true if the database supports Update Cascade.
	 *
	 ...
	 * LIMIT ?' syntax.
	 *
	 * The default implementation returns \c Limit.
	 */
