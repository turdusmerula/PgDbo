 ...
 */
#ifndef DBO_QUERY_IMPL_H_
#define DBO_QUERY_IMPL_H_
...
extern std::string
completeQuerySelectSql(const std::string& sql, const std::string& where, const std::string& groupBy, const std::string& having, const std::string& orderBy, int limit, int offset, const std::vector<FieldInfo>& fields, LimitQuery useRowsFromTo);

extern std::string
createQuerySelectSql(const std::string& from, const std::string& where, const std::string& groupBy, const std::string& having, const std::string& orderBy, int limit, int offset, const std::vector<FieldInfo>& fields, LimitQuery useRowsFromTo);

extern std::string
createWrappedQueryCountSql(const std::string& query, bool requireSubqueryAlias);

extern std::string
createQueryCountSql(const std::string& query, const std::string& from, const std::string& where, const std::string& groupBy, const std::string& having, const std::string& orderBy, int limit, int offset, LimitQuery useRowsFromTo, bool requireSubqueryAlias);

extern void
substituteFields(const SelectFieldList& list, const std::vector<FieldInfo>& fs, std::string& sql, int offset);

extern void
parseSql(const std::string& sql, SelectFieldLists& fieldLists, bool& simpleSelectCount);
...
template<class Result>
QueryBase<Result>::QueryBase(Session& session, const std::string& table, const std::string& where)
		: session_(&session)
{
...
template<class Result>
std::pair<SqlStatement *, SqlStatement *> QueryBase<Result>::statements(const std::string& where, const std::string& groupBy, const std::string& having, const std::string& orderBy, int limit, int offset) const
{
	...
	if(selectFieldLists_.empty())
	{
		...
		sql = Impl::createQuerySelectSql(sql_, where, groupBy, having, orderBy, limit, offset, fs, this->session_->limitQueryMethod_);
		statement = this->session_->getOrPrepareStatement(sql);

		if(simpleCount_)
			sql = Impl::createQueryCountSql(sql, sql_, where, groupBy, having, orderBy, limit, offset, this->session_->limitQueryMethod_, this->session_->requireSubqueryAlias_);
		else
			sql = Impl::createWrappedQueryCountSql(sql, this->session_->requireSubqueryAlias_);
	...
	else
	{
		...
		sql = Impl::completeQuerySelectSql(sql, where, groupBy, having, orderBy, limit, offset, fs, this->session_->limitQueryMethod_);
		...
		if(simpleCount_)
		{
			...
			sql = Impl::createQueryCountSql(sql, from, where, groupBy, having, orderBy, limit, offset, this->session_->limitQueryMethod_, this->session_->requireSubqueryAlias_);
		}
		else
			sql = Impl::createWrappedQueryCountSql(sql, this->session_->requireSubqueryAlias_);
...
template<class Result>
void QueryBase<Result>::fieldsForSelect(const SelectFieldList& list, std::vector<FieldInfo>& result) const
{
...
template<class Result>
Query<Result, DirectBinding>::Query(Session& session, const std::string& table, const std::string& where)
...
void Query<Result, DirectBinding>::prepareStatements() const
{
	...
	boost::tie(this->statement_, this->countStatement_) = this->statements(std::string(), std::string(), std::string(), std::string(), -1, -1);
...
template<class Result>
Query<Result, DynamicBinding>::Query(Session& session, const std::string& table, const std::string& where)
		: Impl::QueryBase<Result>(session, table, where),
				limit_(-1),
				offset_(-1)
{
...
template<class Result>
Query<Result, DynamicBinding>::Query(const Query<Result, DynamicBinding>& other)
...
Query<Result, DynamicBinding>::operator=(const Query<Result, DynamicBinding>& other)
{
...
collection<Result> Query<Result, DynamicBinding>::resultList() const
{
	...
	boost::tie(statement, countStatement) = this->statements(where_, groupBy_, having_, orderBy_, limit_, offset_);
...
template<class Result>
void Query<Result, DynamicBinding>::bindParameters(SqlStatement *statement) const
{
