 ...
 */
#ifndef DBO_DBO_PTR_IMPL_H_
#define DBO_DBO_PTR_IMPL_H_
...
void setAutogeneratedId(MetaDbo<C>& dbo, const T& currentId, long long id)
{
}

template<class C>
void setAutogeneratedId(MetaDbo<C>& dbo, const long long& currentId, long long id)
{
	dbo.setId(id);
}
...
template<class C>
long long asIdType(const ptr<C>& ptr, long long id, const long long& invalidId)
{
	return id;
}
}
...
MetaDbo<C>::~MetaDbo()
{
	...
	if((!isOrphaned()) && Session::current())
		Session::current()->prune(this);
...
{
	return Session::current()->template getMapping<C>();
}
...
void MetaDbo<C>::flush()
{
	...
	if(state_&NeedsDelete)
	{
		...
		try
		{
			Session::current()->implDelete(*this);
	...
	else if(state_&Saving)
	{
		 ...
		 */
		// throw Exception("dbo::ptr::flush(): circular dependency detected!");
	}
}
...
{
	Impl::MappingInfo *mapping = Session::current()->template getMapping<C>();

	SaveBaseAction action(*this, *mapping, statement, column);

	field(action, id_, mapping->naturalIdFieldName, mapping->naturalIdFieldSize);
...
void MetaDbo<C>::doTransactionDone(bool success)
{
	...
	if(obj_)
		Session::current()->implTransactionDone(*this, success);
...
template<class C>
MetaDbo<C>::MetaDbo(const IdType& id, int version, int state, Session& session, C *obj)
...
{
	if(ptr.obj_ && Session::current())
		return o << "[" << Session::current()->template tableName<C>() << ": " << ptr.id() << "]" ;
	else
		return o << "[null]" ;
}
...
template<class C>
void query_result_traits<ptr<C> >::getFields(Session& session, std::vector<std::string> *aliases, std::vector<FieldInfo>& result)
{
...
template<class C>
ptr<C> query_result_traits<ptr<C> >::load(Session& session, SqlStatement& statement, int& column)
{
	return session.template load<C>(&statement, column);
}

template<class C>
void query_result_traits<ptr<C> >::getValues(const ptr<C>& ptr, std::vector<boost::any>& values)
{
...
template<class C>
void query_result_traits<ptr<C> >::setValue(const ptr<C>& ptr, int& index, const boost::any& value)
{
