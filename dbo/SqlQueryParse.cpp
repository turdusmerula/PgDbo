...
void parseSql(const std::string& sql, SelectFieldLists& fieldLists, bool& simpleSelectCount)
{
	fieldLists.clear();
	...
	typedef std::vector<boost::iterator_range<std::string::iterator> > SplitVector;
	...
	for(unsigned i = 0 ; i<aliases.size() ; ++i)
	{
		...
		SelectField f;
		f.begin = aliasStart+(aliases[i].begin()-aliasStr.begin())+alias.size()-a1.size();
