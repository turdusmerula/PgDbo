 ...
 */
#ifndef DBO_SESSION_IMPL_H_
#define DBO_SESSION_IMPL_H_
...
template<class C>
ptr<C> Session::load(const typename dbo_traits<C>::IdType& id, bool forceReread)
{
...
void Session::implDelete(MetaDbo<C>& dbo)
{
	...
	if(versioned)
	{
		...
		if(modifiedCount!=1)
			throw StaleObjectException(boost::lexical_cast<std::string>(dbo.id()), version);
	}
}
