 ...
 */
#ifndef DBO_COLLECTION_H_
#define DBO_COLLECTION_H_
 ...
 *
 * \code
 * void iterateChildren(wtdbo::ptr<Comment> comment)
 * {
 *     typedef wtdbo::collection<wtdbo::ptr<Comment> > Comments;
 ...
 * container. Note that this is no big overhead since wtdbo pointers
 * are lightweight.
 *
 * \code
 * void iterateChildren(wtdbo::ptr<Comment> comment)
 * {
 *     typedef std::vector<wtdbo::ptr<Comment> > Comments;
 *
...
private:
	...
	collection(Session *session, SqlStatement *selectStatement, SqlStatement *countStatement);

	void setRelationData(MetaDboBase *dbo, const std::string *sql, Impl::SetInfo *info);
