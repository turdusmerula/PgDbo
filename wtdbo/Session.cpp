...
SetInfo::SetInfo(const char *aTableName, RelationType aType, const std::string& aJoinName, const std::string& aJoinSelfId, int someFkConstraints)
...
MetaDboBase *MappingInfo::load(Session& session, SqlStatement *statement, int& column)
{
	throw Exception("Not to be done.");
}
...
Session::JoinId::JoinId(const std::string& aJoinIdName, const std::string& aTableIdName, const std::string& aSqlType)
...
Session::~Session()
{
	if(!dirtyObjects_.empty())
		std::cerr<<"Warning: wtdbo::Session exiting with "<<dirtyObjects_.size()<<" dirty objects"<<std::endl;
...
void Session::prepareStatements(Impl::MappingInfo *mapping)
{
	...
	if(!mapping->surrogateIdFieldName)
	{
		...
		if(firstField)
			throw Exception("Table "+std::string(mapping->tableName)+" is missing a natural id defined with wtdbo::id()");
	}
	else
	...
	for(unsigned i = 0 ; i<mapping->sets.size() ; ++i)
	{
		...
		case ManyToMany:
			...
			std::vector<JoinId> otherJoinIds = getJoinIds(otherMapping, info.joinOtherId);
			...
			for(unsigned i = 0 ; i<otherJoinIds.size() ; ++i)
			{
				if(i!=0)
					sql<<" and ";
				sql<<"\""<<joinName<<"\".\""<<otherJoinIds[i].joinIdName<<"\" = \""<<tableName<<"\".\""<<otherJoinIds[i].tableIdName<<"\"";
			}
			...
			std::vector<JoinId> selfJoinIds = getJoinIds(mapping, info.joinSelfId);
...
std::string Session::constraintName(const char *tableName, std::string foreignKeyName)
{
...
void Session::createTable(Impl::MappingInfo *mapping, std::set<std::string>& tablesCreated, std::ostream *sout, bool createConstraints)
{
	...
	if(mapping->surrogateIdFieldName)
	{
		sql<<"  \""<<mapping->surrogateIdFieldName<<"\" "<<connection(false)->autoincrementType()<<" primary key "<<connection(false)->autoincrementSql()<<"";
		firstField = false;
	}
	...
	if(mapping->versionFieldName)
	{
		...
		sql<<"  \""<<mapping->versionFieldName<<"\" "<<sql_value_traits<int>::type(0, 0);
	...
	if(mapping->surrogateIdFieldName)
	{
		...
		std::vector<std::string> sql = connection(false)->autoincrementCreateSequenceSql(tableName, idFieldName);

		executeSql(sql, sout);
	}
}

void Session::createRelations(Impl::MappingInfo *mapping, std::set<std::string>& tablesCreated, std::ostream *sout)
{
	for(unsigned i = 0 ; i<mapping->sets.size() ; ++i)
	{
...
std::string Session::constraintString(Impl::MappingInfo *mapping, const FieldInfo& field, unsigned fromIndex, unsigned toIndex)
{
	std::stringstream sql;
	...
	sql<<") references \""<<Impl::quoteSchemaDot(field.foreignKeyTable())<<"\" ("<<otherMapping->primaryKeys()<<")";
...
unsigned Session::findLastForeignKeyField(Impl::MappingInfo *mapping, const FieldInfo& field, unsigned index)
{
...
void Session::createJoinTable(const std::string& joinName, Impl::MappingInfo *mapping1, Impl::MappingInfo *mapping2, const std::string& joinId1, const std::string& joinId2, int fkConstraints1, int fkConstraints2, std::set<std::string>& tablesCreated, std::ostream *sout)
{
...
void Session::createJoinIndex(Impl::MappingInfo& joinTableMapping, Impl::MappingInfo *mapping, const std::string& joinId, const std::string& foreignKeyName, std::ostream *sout)
{
	std::stringstream sql;
...
std::vector<Session::JoinId> Session::getJoinIds(Impl::MappingInfo *mapping, const std::string& joinId)
{
...
void Session::addJoinTableFields(Impl::MappingInfo& result, Impl::MappingInfo *mapping, const std::string& joinId, const std::string& keyName, int fkConstraints)
{
	std::vector<JoinId> joinIds = getJoinIds(mapping, joinId);

	for(unsigned i = 0 ; i<joinIds.size() ; ++i)
		result.fields.push_back(FieldInfo(joinIds[i].joinIdName, &typeid(long long), joinIds[i].sqlType, mapping->tableName, keyName, FieldInfo::NaturalId|FieldInfo::ForeignKey, fkConstraints));
}
		...
		for(ClassRegistry::iterator i = classRegistry_.begin() ; i!=classRegistry_.end() ; ++i)
		{
			...
			for(unsigned j = 0 ; j<mapping->fields.size() ; ++j)
			{
				...
				if(field.isForeignKey())
				{
					...
					sql<<"alter table \""<<table<<"\""<<" drop "<<connection(false)->alterTableConstraintString()<<" "<<constraintName(mapping->tableName, field.foreignKeyName());
...
{
	return std::string(tableName)+":"+boost::lexical_cast<std::string>(statementIdx);
}
...
void Session::getFields(const char *tableName, std::vector<FieldInfo>& result)
{
	...
	if(mapping->surrogateIdFieldName)
		result.push_back(FieldInfo(mapping->surrogateIdFieldName, &typeid(long long), longlongType_, FieldInfo::SurrogateId|FieldInfo::NeedsQuotes));

	if(mapping->versionFieldName)
		result.push_back(FieldInfo(mapping->versionFieldName, &typeid(int), intType_, FieldInfo::Version|FieldInfo::NeedsQuotes));
