...
public:
	...
	virtual void execute()
	{
		...
		if(!result_)
		{
			...
			{
				if(params_[i].oid!=Param::OIDDefault||params_[i].isbinary==true)
				{
					...
					{
						paramTypes_[j] = params_[j].oid;
						paramFormats_[j] = (params_[j].oid!=Param::OIDDefault||params_[i].isbinary==true)?1:0;
						paramLengths_[j] = 0;
					}
			...
			result_ = PQprepare(conn_.connection(), name_, sql_.c_str(), paramTypes_?params_.size():0, (Oid *)paramTypes_);
		...
		for(unsigned i = 0 ; i<params_.size() ; ++i)
		{
			if(params_[i].isnull)
				paramValues_[i] = 0;
			else if(params_[i].oid!=Param::OIDDefault)
			{
		...
		PQclear(result_);
		result_ = PQexecPrepared(conn_.connection(), name_, params_.size(), paramValues_, paramLengths_, paramFormats_, 0);
	...
	virtual bool getResult(int column, long long *value)
	{
		...
		*value = boost::lexical_cast<long long>(PQgetvalue(result_, row_, column));
	...
	virtual bool getResult(int column, boost::posix_time::ptime *value, SqlDateTimeType type)
	{
		...
		if(type==SqlDate)
			*value = boost::posix_time::ptime(boost::gregorian::from_string(v), boost::posix_time::hours(0));
		else
		{
			/*
			...
			if(v.size()>=3&&std::strchr("+-", v[v.size()-3]))
			{
				...
				*value = boost::posix_time::time_from_string(v.substr(0, v.size()-3))-offset;
			}
			else
	...
	virtual bool getResult(int column, boost::posix_time::time_duration *value)
	{
		...
		*value = boost::posix_time::time_duration(boost::posix_time::duration_from_string(v));
...
std::vector<std::string> Postgres::autoincrementCreateSequenceSql(const std::string &table, const std::string &id) const
{
	return std::vector<std::string>();
}

std::vector<std::string> Postgres::autoincrementDropSequenceSql(const std::string &table, const std::string &id) const
{
	return std::vector<std::string>();
}
