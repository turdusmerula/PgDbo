...
{
	boost::iterator_range<std::string::const_iterator> i = boost::ifind_first(s, needle);
	if(!i)
...
void addGroupBy(std::string& result, const std::string& groupBy, const std::vector<FieldInfo>& fields)
{
...
std::string addLimitQuery(const std::string& sql, int limit, int offset, LimitQuery limitQueryMethod)
{
	...
	case Rownum:
		if(limit!=-1&&offset==-1)
			...
			result = " select * from ( select row_.*, rownum rownum2 from ( "+result+" ) row_ where rownum <= ?) where rownum2 > ?";
...
std::string completeQuerySelectSql(const std::string& sql, const std::string& where, const std::string& groupBy, const std::string& having, const std::string& orderBy, int limit, int offset, const std::vector<FieldInfo>& fields, LimitQuery limitQueryMethod)
{
	std::string result = sql;
...
std::string createQuerySelectSql(const std::string& from, const std::string& where, const std::string& groupBy, const std::string& having, const std::string& orderBy, int limit, int offset, const std::vector<FieldInfo>& fields, LimitQuery limitQueryMethod)
{
...
std::string createWrappedQueryCountSql(const std::string& query, bool requireSubqueryAlias)
{
...
std::string createQueryCountSql(const std::string& query, const std::string& from, const std::string& where, const std::string& groupBy, const std::string& having, const std::string& orderBy, int limit, int offset, LimitQuery limitQueryMethod, bool requireSubqueryAlias)
{
	/*
	 ...
	 * parameter.
	 */
	if(!groupBy.empty()||ifind(from, "group by")!=std::string::npos||!having.empty()||ifind(from, "having")!=std::string::npos||!orderBy.empty()||ifind(from, "order by")!=std::string::npos||limit!=-1||offset!=-1)
...
void substituteFields(const SelectFieldList& list, const std::vector<FieldInfo>& fs, std::string& sql, int offset)
{
	for(unsigned i = 0, j = 0 ; j<list.size() ; ++j)
	{
		if(fs[i].isFirstDboField())
		{
			...
			for(; ;)
			{
				...
				++i;
				if(i>=fs.size()||fs[i].qualifier().empty()||fs[i].isFirstDboField())
					break;
			}
