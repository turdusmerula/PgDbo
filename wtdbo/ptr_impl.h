...
void MetaDbo<C>::flush()
{
	...
	else if(state_&Saving)
	{
		 ...
		 */
		// throw Exception("wtdbo::ptr::flush(): circular dependency detected!");
	}
}
...
const C *ptr<C>::operator->() const
{
	...
	if(!v)
		throw Exception("wtdbo::ptr: null dereference");
...
std::ostream& operator<<(std::ostream& o, const ptr<C>& ptr)
{
	if(ptr.obj_&&ptr.obj_->session())
		return o<<"["<<ptr.obj_->session()->template tableName<C>()<<": "<<ptr.id()<<"]";
	else
		return o<<"[null]";
}
