 ...
 */
#ifndef DBO_DBACTION_IMPL_H_
#define DBO_DBACTION_IMPL_H_
...
namespace Impl
{
extern std::string createJoinName(RelationType type, const char *c1, const char *c2);
}
...
void InitSchema::actId(V& value, const std::string& name, int size)
{
	...
	if(mapping_.surrogateIdFieldName)
		throw Exception("Error: wtdbo::id() called for class C "
				"with surrogate key: "
				"wtdbo::dbo_traits<C>::surrogateIdField() != 0");
...
template<class C>
void InitSchema::actId(ptr<C>& value, const std::string& name, int size, int fkConstraints)
{
	mapping_.naturalIdFieldName = name;
	mapping_.naturalIdFieldSize = size;
...
void InitSchema::act(const FieldRef<V>& field)
{
	...
	if(!foreignKeyName_.empty())
		// Foreign key
		mapping_.fields.push_back(FieldInfo(field.name(), &typeid(V), field.sqlType(session_), foreignKeyTable_, foreignKeyName_, flags|FieldInfo::ForeignKey, fkConstraints_));
	else
		// Normal field
...
void InitSchema::actWeakPtr(const WeakPtrRef<C>& field)
{
	...
	mapping_.sets.push_back(Impl::SetInfo(joinTableName, ManyToOne, joinName, std::string(), 0));
}
...
void InitSchema::actCollection(const CollectionRef<C>& field)
{
	...
	std::string joinName = field.joinName();
	if(joinName.empty())
		joinName = Impl::createJoinName(field.type(), mapping_.tableName, joinTableName);

	mapping_.sets.push_back(Impl::SetInfo(joinTableName, field.type(), joinName, field.joinId(), field.fkConstraints()));
}
...
template<class C>
void DropSchema::actId(ptr<C>& value, const std::string& name, int size, int fkConstraints)
{
}
...
void DropSchema::actWeakPtr(const WeakPtrRef<C>& field)
{
	...
	{
		DropSchema action(session_, *session_.getMapping(tableName), tablesDropped_);
		C dummy;
		action.visit(dummy);
	}
}
...
template<class C>
LoadDbAction<C>::LoadDbAction(MetaDbo<C>& dbo, Session::Mapping<C>& mapping, SqlStatement *statement, int& column)
...
void LoadDbAction<C>::visit(C& obj)
{
	...
	if(!continueStatement)
	{
		...
		{
			throw ObjectNotFoundException(boost::lexical_cast<std::string>(dbo_.id()));
		}
	}
	...
	if(!continueStatement&&statement_->nextRow())
		throw Exception("Dbo load: multiple rows for id "+boost::lexical_cast<std::string>(dbo_.id())+" ??");
...
template<class C>
template<class D>
void LoadDbAction<C>::actId(ptr<D>& value, const std::string& name, int size, int fkConstraints)
{
	...
	case Sets:
		if(field.type()==ManyToMany)
		{
			...
			if(activity)
			{
				...
				// Sql insert
				int statementIdx = Session::FirstSqlSelectSet+setStatementIdx()+1;
				...
				statement = session()->getStatement(mapping().tableName, statementIdx);
				{
					...
					for(typename std::set<ptr<C> >::iterator i = inserted.begin() ; i!=inserted.end() ; ++i)
					{
				...
				{
					ScopedStatementUse use(statement);
					for(typename std::set<ptr<C> >::iterator i = erased.begin() ; i!=erased.end() ; ++i)
					{
						MetaDboBase *dbo2 = dynamic_cast<MetaDboBase *>(i->obj());
				...
				activity->transactionInserted.insert(activity->inserted.begin(), activity->inserted.end());
				activity->transactionErased.insert(activity->erased.begin(), activity->erased.end());
	 ...
	 */
	{
		...
		if(!isInsert_)
		{
			...
			if(mapping().versionFieldName)
			{
				...
				statement_->bind(column_++, dbo_.version()+(dbo_.savedInTransaction()?1:0));
			}
		}
...
template<class C>
template<class D>
void SaveDbAction<C>::actId(ptr<D>& value, const std::string& name, int size, int fkConstraints)
{
	actPtr(PtrRef<D>(value, name, size, fkConstraints));
...
template<class C>
void TransactionDoneAction::actId(ptr<C>& value, const std::string& name, int size, int fkConstraints)
{
	actPtr(PtrRef<C>(value, name, size, fkConstraints));
}
...
template<class C>
void SetReciproceAction::actId(ptr<C>& value, const std::string& name, int size, int fkConstraints)
{
	actPtr(PtrRef<C>(value, name, size, fkConstraints));
}
...
template<class C>
void ToAnysAction::actId(ptr<C>& value, const std::string& name, int size, int fkConstraints)
{
	actPtr(PtrRef<C>(value, name, size, fkConstraints));
}
...
template<class C>
void FromAnyAction::actId(ptr<C>& value, const std::string& name, int size, int fkConstraints)
{
	actPtr(PtrRef<C>(value, name, size, fkConstraints));
}
