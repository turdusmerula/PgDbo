...
protected:
	...
	void fieldsForSelect(const SelectFieldList& list, std::vector<FieldInfo>& result) const;
	std::pair<SqlStatement *, SqlStatement *>
	statements(const std::string &where, const std::string &groupBy, const std::string &having, const std::string &orderBy, int limit, int offset) const;
	Session &session() const;

	QueryBase();
	QueryBase(Session& session, const std::string& sql);
	QueryBase(Session& session, const std::string& table, const std::string& where);
...
/*! \class DirectBinding Dbo/Query Dbo/Query
 *
...
/*! \class DynamicBinding Dbo/Query Dbo/Query
 *
...
/*! \class Query Dbo/Query Dbo/Query
 ...
 * Usage example:
 * \code
 * typedef wtdbo::ptr<Account> AccountPtr;
 * typedef wtdbo::collection<AccountPtr> Accounts;
 *
 * wtdbo::Query<AccountPtr> query = session.find<Account>().where("balance > ?").bind(100000);
...
public:
	...
	template <class U, class V> friend class Query ;
	...
	/**
	 * Accept a query filter from any type
	 */
	template<class T>
	Query<Result, DynamicBinding>& query(const Query<T, DynamicBinding>& other) ;
...
//template<class T>
//template<class U>
//void query(Query<T, DynamicBinding>& left, const Query<U, DynamicBinding>& other)
//{
//	left.where_ = other.where_;
//	left.groupBy_ = other.groupBy_;
//	left.having_ = other.having_;
//	left.orderBy_ = other.orderBy_;
//	left.limit_ = other.limit_;
//	left.offset_ = other.offset_;
//
//	for(unsigned i = 0 ; i<other.parameters_.size() ; ++i)
//		left.parameters_.push_back(other.parameters_[i]->clone());
//
//}

template<class Result>
template<typename T>
Query<Result, DynamicBinding>& Query<Result, DynamicBinding>::query(const Query<T, DynamicBinding>& other)
{
	where_ = other.where_;
