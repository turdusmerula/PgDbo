 ...
 */
#ifndef DBO_FIELD_IMPL_H_
#define DBO_FIELD_IMPL_H_
...
template<class C>
CollectionRef<C>::CollectionRef(collection<ptr<C> >& value, RelationType type, const std::string& joinName, const std::string& joinId, int fkConstraints)
		: value_(value),
				joinName_(joinName),
				joinId_(joinId),
				type_(type),
...
{
	static void loadLazy(ptr<C>& p, typename dbo_traits<C>::IdType id, Session *session)
	{
	}
};
	...
	static void loadLazy(ptr<C>& p, typename dbo_traits<C>::IdType id, Session *session)
	{
		if(!(id==dbo_traits<C>::invalidId()))
		{
			if(session)
				...
				throw Exception("Could not load referenced wtdbo::ptr, no session?");
		}
	}
};
...
void id(A& action, ptr<C>& value, const std::string& name, ForeignKeyConstraint constraint, int size)
{
...
template<class A, class C>
void belongsTo(A& action, ptr<C>& value, const std::string& name, ForeignKeyConstraint constraint, int size)
{
	belongsToImpl(action, value, name, constraint.value(), size);
}

template<class A, class C>
void belongsTo(A& action, ptr<C>& value, ForeignKeyConstraint constraint, int size)
{
...
template<class A, class C>
void hasMany(A& action, collection<ptr<C> >& value, RelationType type, const std::string& joinName)
{
	action.actCollection(CollectionRef<C>(value, type, joinName, std::string(), Impl::FKNotNull|Impl::FKOnDeleteCascade));
}

template<class A, class C>
void hasMany(A& action, collection<ptr<C> >& value, RelationType type, const std::string& joinName, const std::string& joinId, ForeignKeyConstraint constraint)
{
	if(type!=ManyToMany)
		throw Exception("hasMany() with named joinId only for a ManyToMany relation");

	action.actCollection(CollectionRef<C>(value, type, joinName, joinId, constraint.value()));
}

}

#endif // DBO_FIELD_IMPL_H_
