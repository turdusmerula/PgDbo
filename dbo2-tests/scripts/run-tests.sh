...
root_path=$(echo ${xpl_path} | sed "s#\(.*\)/dbo2-tests/.*#\1#g")
cd $xpl_path
...
if [ $ddd ]
then
	ddd --debugger "gdb -x ${conf_path}/gdb.ini --args ${bin_path}/dbo2-tests $filtargs"
elif [ $asan ]
then
	...
	( ${bin_path}/dbo2-tests $filtargs 2>&1 ) | /opt/asan_symbolize.py 
elif [ $lsan ]
then
...
then
	...
	${bin_path}/dbo2-tests $filtargs

	/opt/gperftools/src/pprof --text --stacks ${bin_path}/dbo2-tests /tmp/testHeapProfile.0003.heap	
	
	echo "---- diff between /tmp/testHeapProfile.0001.heap and /tmp/testHeapProfile.0002.heap"
	/opt/gperftools/src/pprof --text --stacks --base=/tmp/testHeapProfile.0001.heap ${bin_path}/dbo2-tests /tmp/testHeapProfile.0002.heap	
elif [ $valgrind ]
then
...
then
	...
	valgrind --tool=callgrind ${bin_path}/dbo2-tests $filtargs
elif [ $ftrace ]
then
	echo "[ Run tests with ftrace ]"
	LD_PRELOAD=/opt/ftrace/Build/lib/libftracelib.so FTRACE=${conf_path}/ftrace.fcf ${bin_path}/dbo2-tests $filtargs
else
	${bin_path}/dbo2-tests $filtargs
fi
