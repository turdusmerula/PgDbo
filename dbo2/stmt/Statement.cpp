...
//	{
//		std::stringstream ss ;
//		ss << "Statement '" << name_ << "' error: statements with a request result cannot be copied" ;
//		ss << " -> " << sql_ ;
...
//	{
//		std::stringstream ss ;
//		ss << "Statement '" << name_ << "' error: assignment needs statements from the same connections" ;
//		ss << " -> " << sql_ ;
//		throw Exception(ss.str()) ;
//	}
//
...
void Statement::prepare()
{
	...
	if(hashname_)
	{
		...
		{
			// Statement already exist,exit ok
			PQclear(result) ;
			prepared_ = true ;
			return ;
		}
	...
	if(err!=PGRES_COMMAND_OK && err!=PGRES_TUPLES_OK)
	{
		PQclear(result) ;
		std::stringstream ss ;
		ss << "Statement '" << name_ << "' preparation failed: " << PQerrorMessage(conn_->conn_) << " needed" ;
		ss << " -> " << sql_ ;
		throw Exception(ss.str()) ;
	}
	PQclear(result) ;
...
size_t Statement::getPlaceHolders(const std::string& sql, std::map<std::string, std::string>& placeholders)
{
...
void Statement::sql(const std::string& sql)
{
	if(prepared_)
	{
		std::stringstream ss ;
		ss << "Statement '" << name_ << "' already prepared, SQL can not be modified"  ;
		throw Exception(ss.str()) ;
	}
...
void Statement::name(const std::string& name)
{
	hashname_ = false ;
	name_ = name ;
}

void Statement::hashName(bool value)
{
	hashname_ = true ;
	name_ = "" ;
}

void Statement::pg_result_deleter(pg_result* result)
{
	PQclear(result) ;
	result = nullptr ;
}
