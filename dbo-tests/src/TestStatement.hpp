...
dbo::connection TestStatement::db ;
...
TEST_F(TestStatement, TestNamedStatement) {
	dbo::ptr<iSimpleTable> p=dbo::make_ptr<iSimpleTable>() ;
	db.insert(p) ;

	dbo::stmt::PreparedStatement stmt(db, "TestNamedStatement", "select * from \"iSimpleTable\" where id=?") ;
...
TEST_F(TestStatement, TestNameHashStatement) {
	...
	dbo::stmt::PreparedStatement stmt(db, "select * from \"iSimpleTable\" where id=?") ;
	stmt.bind(boost::lexical_cast<std::string>(p.id())) ;
	ASSERT_NO_THROW_V( stmt.prepare() ) ;
	ASSERT_NO_THROW_V( stmt.execute() ) ;
...
TEST_F(TestStatement, TestWrongPrepare) {
	...
	dbo::stmt::PreparedStatement stmt(db, "TestWrongPrepare", "select * from \"iSimpleTable\" where id=? and value1=?") ;
	stmt.bind(boost::lexical_cast<std::string>(p.id())) ;
...
TEST_F(TestStatement, TestDoublePrepareHashed) {
	...
	dbo::stmt::PreparedStatement stmt1(db, "select * from \"iSimpleTable\" where id=? and value1=?") ;
	...
	dbo::stmt::PreparedStatement stmt2(db, "select * from \"iSimpleTable\" where id=? and value1=?") ;
...
TEST_F(TestStatement, TestDoublePrepareNamed) {
	...
	// hashed name statement
	dbo::stmt::PreparedStatement stmt1(db, "TestDoublePrepareNamed", "select * from \"iSimpleTable\" where id=? and value1=?") ;
	stmt1.bind(boost::lexical_cast<std::string>(p.id())) ;
	stmt1.bind(boost::lexical_cast<std::string>("")) ;
	...
	// Prepare same statement
	dbo::stmt::PreparedStatement stmt2(db, "TestDoublePrepareNamed", "select * from \"iSimpleTable\" where id=? and value1=?") ;
	stmt2.bind(boost::lexical_cast<std::string>(p.id())) ;
	stmt2.bind(boost::lexical_cast<std::string>("")) ;
...
TEST_F(TestStatement, TestMixedParameters) {
	...
	dbo::stmt::PreparedStatement stmt(db, "select * from \"iSimpleTable\" where value1=$1 and id=? and value2=$2 and value3=$1") ;
	stmt.bind("v") ;
	stmt.bind("w") ;
	stmt.bind(boost::lexical_cast<std::string>(p.id())) ;
...
TEST_F(TestStatement, TestWrongBind) {
	...
	dbo::stmt::PreparedStatement stmt(db, "select * from \"iSimpleTable\" where id=? and value1=?") ;
	stmt.bind(boost::lexical_cast<std::string>(p.id())) ;
	stmt.bind("") ;
